...
import { Grid, Step, StepLabel, Stepper, Typography, Box, DialogContent, IconButton } from "@mui/material";
...
import { finalizeCandidate, getCandidateDetails } from "@redux/Redux/Actions/Candidate";
...
export default function Edit_Candidate() {
  ...
  const getCandidateAdditionalDetailsData = useAppSelector(
    (state: any) =>
    state?.partner?.isGetCandidateDetails?.getCandidateDetailsData?.data
    );
    console.log('====================================');
    console.log("getCandidateAdditionalDetailsData",getCandidateAdditionalDetailsData);
    console.log('====================================');
  ...
  const FinalizeCandidateComplete = async () => {
    setFullPageLoader(true);
    const body = {
      requisitionId: router?.query?.requisitionId,
      candidateId: router?.query?.candidateId
    }
    let finalizeCandidateData = await dispatch(finalizeCandidate(body))
    if(finalizeCandidateData?.payload?.code === 1){
      handleDialogOpen()
      ...
      >
        ...
        <Grid item md={3} xs={2} spacing={2} flexWrap="wrap">
          ...
          >
            ...
            <ButtonArrow
              onClick={activeStep >= 3 ? FinalizeCandidateComplete : handleNext}
              text={activeStep >= 3 ? "Finish": "Next"}
              borderRadius="30px"
              height="40px"
              width="100px"
              disabled={!getCandidateAdditionalDetailsData}
            />
          </Box>
        </Grid>
      </Box>
        ...
        <Grid container spacing={0}>
          ...
          <Grid item lg={8} md={9} xs={12}>
            <Box padding={"20px 0px 20px 30px"} className={classes.overFlowScroll}>
              ...
              {activeStep === 0 && (
                <Box marginTop={2}>
                  <BasicDetails jdData={jdData} getCandidateAdditionalDetailsData={getCandidateAdditionalDetailsData}/>
                </Box>
              )}
              {activeStep === 1 && (
                <Box marginTop={2}>
                  <AddDocument  getCandidateAdditionalDetailsData={getCandidateAdditionalDetailsData} onClick={handleStep(2)}/>
                </Box>
              )}
              {activeStep === 2 && (
                <Box marginTop={2}>
                  <Screening  getCandidateAdditionalDetailsData={getCandidateAdditionalDetailsData} onClick={handleStep(3)}/>
                </Box>
              )}
              {activeStep === 3 && (
                <Box>
                  <Preview jdData={jdData} getCandidateAdditionalDetailsData={getCandidateAdditionalDetailsData} />
                </Box>
              )}
            </Box>
          </Grid>
                ...
                <CustomDialog isOpen={isOpen} handleClose={handleDialogClose}>
                  ...
                  >
                    ...
                    <Typography
                      ...
                      mt={1}
                    >
                      Candidate has been created sucessfully
                    </Typography>
                  </DialogContent>
                </CustomDialog>
              )}
